...
**マルチスレッド比較**（Shrake-Rupley）：

| 構造 | スレッド数 | Zig (ms) | RustSASA (ms) | Zig高速化 |
|------|----------:|---------:|--------------:|----------:|
...
**概要**: シングルスレッドでは同等だが、マルチスレッドではZigが**1.8x-2.3x高速**。RustSASAはスレッドスケーリングが限定的（t=1→10で1.4x）、Zigは良好にスケール（t=1→10で2.7x）。RustSASAはShrake-Rupleyアルゴリズムのみ対応。
