...
**概要**: 両アルゴリズムともFreeSASA Cより高速。Shrake-Rupleyは**1.1x-2.1x高速**、Lee-Richardsは**1.7x高速**。構造サイズが大きいほど高速化率が向上。

### RustSASAとの比較

[RustSASA](https://github.com/maxall41/RustSASA)はバッチ処理に最適化されたRust実装です（内部でf32精度を使用）。

**シングルスレッド比較**（n_points=100、Shrake-Rupley）：

| 構造 | 原子数 | Zig (ms) | RustSASA (ms) | FreeSASA C (ms) |
|------|-------:|---------:|--------------:|----------------:|
...
シングルスレッドでは3実装とも同等の性能。

**構造サイズ別マルチスレッド比較**（10スレッド、Shrake-Rupley、約10万構造）：

| サイズ | 構造数 | Zig vs FreeSASA | Zig vs RustSASA |
|--------|-------:|----------------:|----------------:|
...
**概要**: 構造サイズが大きいほどZigの優位性が増大。大規模構造（20k原子以上）ではZigが FreeSASA C、RustSASA 双方より**1.9x-2.3x高速**。RustSASAはShrake-Rupleyアルゴリズムのみ対応。

**バッチ処理比較**（10,000ファイル、10スレッド）：
...
Zig f32はRustSASAより**3%高速**。Zig f64（倍精度）でもRustSASAと同等の速度を達成。
