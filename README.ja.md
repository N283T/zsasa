...
約10万PDB構造でベンチマーク。手法と詳細は[docs/benchmark.md](docs/benchmark.md)を参照。

**構造サイズ別高速化率**（Shrake-Rupley、10スレッド）：

| サイズ | Zig vs FreeSASA C | Zig vs RustSASA |
|--------|------------------:|----------------:|
| 小規模 (<2k) | 0.9x-1.3x | 1.0x-1.5x |
| 中規模 (2k-20k) | 1.4x-1.7x | 1.7x-2.0x |
| 大規模 (20k+) | 1.9x-2.3x | 2.1x-2.3x |

**主な結果**：
- シングルスレッド：全実装で同等
- マルチスレッド：大規模構造（20k原子以上）でZigが**1.9x-2.3x高速**
- バッチ処理：Zig f32はRustSASAと同等のスループット（約58ファイル/秒）
