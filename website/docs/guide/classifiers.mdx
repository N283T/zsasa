...
Custom classifiers are an advanced feature for studies that require a specific radius table. For most structures, keep the default CCD classifier because it covers standard residues and uses CCD bond topology for ligands, cofactors, and modified residues.

Custom classifier files must be TOML files with the `.toml` extension.
...
Older custom classifier text files are no longer supported as input. Convert type definitions and atom mappings to TOML:
