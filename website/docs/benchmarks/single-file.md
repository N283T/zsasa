...
Single-file benchmarks focus on large structures and parser-heavy cases that are hidden by directory-level averages. The pinned paper-era suite uses eight protein-only PDB inputs, 100 sphere points, and 10 threads for the headline results.
...
![Single-file runtime and peak RSS](pathname:///zsasa/assets/benchmarks/paper/fig5_single_runtime_rss.png)

**Figure 1. Single-structure runtime and peak memory versus atom count.** `zsasa` remains faster than FreeSASA and RustSASA across the curated subset while using less memory.
...
![Single-file parse and SASA timing](pathname:///zsasa/assets/benchmarks/paper/fig6_single_parse_sasa.png)

**Figure 2. Parse time versus SASA-kernel time.** On the largest assembly, `zsasa` f64 spent about 708 ms parsing and 3,754 ms in SASA calculation; bitmask reduced the SASA component to about 2,861 ms.
...
**Figure 3. Runtime versus thread count across the curated single-file subset.** Thread scaling depends on structure size and parser behavior; the largest structures benefit most from parallel SASA calculation.
