...
- [ ] PDB パーサーに水素原子スキップオプション追加 (`skip_hydrogens: bool = true`)
- [ ] mmCIF パーサーにも同様のオプション追加
- [ ] CLI に `--include-hydrogens` / `--include-hetatm` フラグ追加
- [ ] デフォルト: 水素除外、HETATM 除外 (FreeSASA/RustSASA と一致)
- [ ] バッチモード (`batch.zig`) にもフラグ伝播
- [ ] テスト追加 (水素あり/なし PDB で atom count が変わることを確認)
- [ ] docs/cli.md 更新
