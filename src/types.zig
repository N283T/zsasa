...
pub const AtomInput = struct {
    ...
    /// Create a filtered copy containing only atoms matching the given chain ID.
    /// Caller must call deinit() on the returned AtomInput.
    pub fn filterByChain(self: AtomInput, target_chain: []const u8) !AtomInput {
        const chains = self.chain_id orelse return error.NoChainInfo;

        // Count matching atoms
        var count: usize = 0;
        for (chains) |chain| {
            if (std.mem.eql(u8, chain, target_chain)) count += 1;
        }

        if (count == 0) return error.NoMatchingAtoms;

        // Allocate arrays
        const x = try self.allocator.alloc(f64, count);
        errdefer self.allocator.free(x);
        const y = try self.allocator.alloc(f64, count);
        errdefer self.allocator.free(y);
        const z = try self.allocator.alloc(f64, count);
        errdefer self.allocator.free(z);
        const r = try self.allocator.alloc(f64, count);
        errdefer self.allocator.free(r);

        // Copy matching atoms
        var idx: usize = 0;
        for (0..self.atomCount()) |i| {
            if (std.mem.eql(u8, chains[i], target_chain)) {
                x[idx] = self.x[i];
                y[idx] = self.y[i];
                z[idx] = self.z[i];
                r[idx] = self.r[i];
        ...
        return AtomInput{
            ...
            // Note: We don't copy optional fields for simplicity
            // as they're not needed for SASA calculation
        };
    }

    /// Get list of unique chain IDs
    pub fn getUniqueChains(self: AtomInput, allocator: std.mem.Allocator) ![][]const u8 {
        const chains = self.chain_id orelse return error.NoChainInfo;
        ...
        for (chains) |chain| {
            var found = false;
            for (unique.items) |u| {
                if (std.mem.eql(u8, u, chain)) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                try unique.append(allocator, chain);
            }
        }
