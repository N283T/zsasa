...
| `classifier_parser.zig` | FreeSASA互換設定ファイルパーサー |
...
分類器を使用するには、入力JSONに`residue`と`atom_name`フィールドが必要：
...
分類器は元の`r`値を上書きし、残基/原子名に基づいて半径を割り当てる。

### 優先順位

`--config`と`--classifier`の両方が指定された場合、`--config`が優先される。
...
FreeSASA互換の設定ファイル形式をサポート。カスタム原子半径を定義可能。
