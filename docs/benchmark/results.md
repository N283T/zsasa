...
複数ファイルを並列処理するバッチモードの比較。PDB 全構造 **238,124 ファイル**を 10 スレッドで処理。
...
**観察:**
- Zig f32 は Rust f32 より **+7%** 高速
- Zig f64（倍精度）でも Rust f32 より **+4%** 高速
- 全件（238k ファイル）での検証により統計的に有意な結果
...
# データセット分布
./benchmarks/scripts/sample.py plot benchmarks/samples/stratified_100k.json
./benchmarks/scripts/sample.py plot benchmarks/samples/stratified_30k.json
