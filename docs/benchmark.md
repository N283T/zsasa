...
|----------------|--------|
| freesasa-zig | `--benchmark` オプションで内部計測 |
| FreeSASA C | Python bindings 経由で `freesasa.calc()` のみ計測 |
...
```bash
# Full benchmark (all structures, both algorithms)
./scripts/benchmark.py

# Custom settings
./scripts/benchmark.py --runs 10 --threads 8

# Single-threaded comparison
./scripts/benchmark.py --threads 1

# Specific structures only
./scripts/benchmark.py --pdbs 1CRN,1UBQ,1A0Q
```

### Requirements

- freesasa-zig binary (`zig build -Doptimize=ReleaseFast`)
- FreeSASA C binary (`freesasa` in PATH)
- Python 3.11+ with dependencies (auto-installed via PEP 723)

Optional:
- RustSASA patched binary in `rust-sasa-bench/target/release/rust-sasa`
